...
| `--hide-thoughts`       | 不在输出消息里渲染思考过程                                                                             |
| `--hide-tools`          | 不在输出消息里渲染工具调用                                                                             |
| `--hide-cancel-button`  | 兼容旧版卡片输出的保留选项；当前普通输出是 post 消息，取消请用 `/cancel`                               |
...
| 回调名       | event_type            | 用途                                         |
| ------------ | --------------------- | -------------------------------------------- |
| 卡片回传交互 | `card.action.trigger` | 用户点击卡片按钮（授权选项等需要交互的卡片） |
